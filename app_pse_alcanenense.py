...
st.set_page_config(page_title="PSE Alcanenense", page_icon="⚽", layout="wide")
...
def classificar_pse(valor):
    if valor == 0: return "Repouso"
    if valor == 1: return "Muito, muito fácil"
    if valor == 2: return "Fácil"
    if valor == 3: return "Moderado"
    if valor == 4: return "Algo difícil"
    if valor in [5, 6]: return "Difícil"
    if valor in [7, 8, 9]: return "Muito difícil"
...
def ligar_google_sheets():
    if "SHEET_ID" not in st.secrets or "gcp_service_account" not in st.secrets:
        return None, "Faltam os Secrets do Google Sheets no Streamlit."
...
<style>
    ...
    .header-box { background-color: #050505; border-radius: 22px; padding: 28px 20px 32px 20px; text-align: center; margin-bottom: 28px; box-shadow: 0px 8px 24px rgba(0,0,0,0.25); }
    .header-title { color: #f5e51b; font-size: 34px; font-weight: 900; margin-top: 10px; margin-bottom: 6px; line-height: 1.15; }
    .header-subtitle { color: white; font-size: 16px; font-weight: 500; }
    h1, h2, h3 { color: #111111; font-weight: 850; }
    .stApp, .stApp p, .stApp label, .stApp span, .stApp div { color: #111111; }
    .header-box, .header-box div, .header-title { color: #f5e51b !important; }
    .header-subtitle { color: #ffffff !important; }
    [data-testid="stSidebar"], [data-testid="stSidebar"] * { color: #f5e51b !important; }
    .stAlert div { color: #111111 !important; }
    .stButton > button, .stDownloadButton > button { background-color: #050505; color: #f5e51b !important; border: none; border-radius: 12px; font-weight: 800; padding: 0.65rem 1.4rem; }
    .stButton > button:hover, .stDownloadButton > button:hover { background-color: #222222; color: #fff27a !important; }
    div[data-testid="stMetric"] { background-color: #ffffff; border-radius: 16px; padding: 14px; box-shadow: 0px 5px 16px rgba(0,0,0,0.10); border-left: 6px solid #111111; }
</style>
...
pagina = st.sidebar.radio("Escolhe a zona:", ["Atleta - Responder", "Treinador - Análise"])
...
if pagina == "Atleta - Responder":
    ...
    with st.form("formulario_pse", clear_on_submit=True):
        ...
        pse = st.select_slider("Qual foi a tua perceção do esforço durante o treino de hoje?", options=list(range(0, 11)), value=5)
...
if pagina == "Treinador - Análise":
    with st.container(border=True):
        ...
        with f1:
            data_escolhida = st.selectbox("Filtrar por data", ["Todas"] + [str(d) for d in datas])
        atletas = sorted(df["Atleta"].dropna().unique())
        with f2:
            atleta_escolhido = st.selectbox("Filtrar por atleta", ["Todos"] + atletas)
        ...
        st.write(f"Foram registadas {total} respostas. A média da PSE foi {media_pse:.1f}, o que representa um nível {nivel_alerta(media_pse)} de esforço percebido. A PSE máxima foi {int(max_pse)} e a mínima foi {int(min_pse)}.")
        if not elevados.empty:
            st.warning("Atletas com PSE elevada: " + ", ".join(elevados["Atleta"].tolist()))
        else:
        ...
        with tab2:
            ...
            if df_f["Data"].nunique() > 1:
                ...
                fig2 = px.line(evolucao, x="Data", y="PSE", markers=True, title="Evolução da média da PSE")
                fig2.update_layout(yaxis_range=[0, 10])
                st.plotly_chart(fig2, use_container_width=True)
        with tab3:
            excel = preparar_excel(df_f)
            st.download_button("Descarregar Excel", data=excel, file_name="respostas_pse.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
